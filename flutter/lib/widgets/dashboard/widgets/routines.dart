...
class _DashboardRoutineWidgetState extends State<DashboardRoutineWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    final dateFormat = DateFormat.yMd(appDateLocale(context));

    // Compute simple progress for current iteration: number of non-rest days
    // scheduled up to today vs total in iteration
    final iterationDays = routine?.dayDataCurrentIteration.where((d) => d.day != null && !(d.day!.isRest)).toList() ?? [];
    final totalWorkDays = iterationDays.length;
    final completedWorkDays = iterationDays.where((d) => !d.date.isAfter(DateTime.now())).length;
      ...
      child: Column(
        children: [
          ListTile(
            ...
            leading: _hasContent && totalWorkDays > 0
                ? SizedBox(
                    width: 56,
                    height: 56,
                    child: Stack(
                      ...
                      children: [
                        CircularProgressIndicator(
                          value: completedWorkDays / totalWorkDays,
                          strokeWidth: 6,
                          backgroundColor: Theme.of(context).colorScheme.surfaceVariant,
                        ),
                        Text('$completedWorkDays/$totalWorkDays', style: Theme.of(context).textTheme.labelLarge),
                      ],
                    ),
                  )
                : Icon(
                    ...
                    Expanded(
                      child: Text(
                        dayData.day == null || dayData.day!.isRest
                            ...
                            : translateWeekdayName(dayData.day!.name, context),

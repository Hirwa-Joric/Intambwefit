...
class _MeasurementChartWidgetFlState extends State<MeasurementChartWidgetFl> {
    ...
    return LineTouchData(
      touchTooltipData: LineTouchTooltipData(
        ...
        getTooltipItems: (touchedSpots) {
          final numberFormat =
              NumberFormat.decimalPattern(appDateLocale(context));

          return touchedSpots.map((touchedSpot) {
            final DateTime date = DateTime.fromMillisecondsSinceEpoch(touchedSpot.x.toInt());
            final dateStr =
                DateFormat.Md(appDateLocale(context)).format(date);
  ...
  LineChartData mainData() {
    final numberFormat = NumberFormat.decimalPattern(appDateLocale(context));
    ...
    return LineChartData(
      ...
      gridData: FlGridData(
        ...
        getDrawingHorizontalLine: (value) {
          return FlLine(color: Theme.of(context).colorScheme.outline.withValues(alpha: 0.25), strokeWidth: 1);
        },
          ...
          sideTitles: SideTitles(
            ...
            getTitlesWidget: (value, meta) {
              ...
              if (DateTime.fromMillisecondsSinceEpoch(meta.min.toInt()).year !=
                ...
                return Text(
                  DateFormat.yMd(appDateLocale(context)).format(date),
                );
              }
              return Text(
                DateFormat.Md(appDateLocale(context)).format(date),
